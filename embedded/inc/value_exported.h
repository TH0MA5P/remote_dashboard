...
bool VALUE_EXPORTED_add_Ptr(char group[sizeof(uint32_t)], char name[sizeof(uint32_t)], uint16_t nb_elem_x,
                            uint16_t nb_elem_y, void *ptr_value, bool readPermission,
                            bool writePermission, value_type_t type, char desc[DESC_SIZE]);

bool VALUE_EXPORTED_add_Fct(char group[sizeof(uint32_t)], char name[sizeof(uint32_t)], uint16_t nb_elem_x,
